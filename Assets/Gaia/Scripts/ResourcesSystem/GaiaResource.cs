    ...
    public class GaiaResource : ScriptableObject
    {
            ...
            switch (resourceType)
            {
                ...
                case GaiaConstants.SpawnerResourceType.TerrainDetail:
                    ...
                    foreach (DetailPrototype proto in terrain.terrainData.detailPrototypes)
                    {
                        ...
                        {
                            if (Gaia.Utils.IsSameTexture(detail.m_detailTexture, proto.prototypeTexture, false))
                            {
                                return localTerrainIdx;
                            }
                            if (Gaia.Utils.IsSameGameObject(detail.m_detailProtoype, proto.prototype, false))
                            {
                                return localTerrainIdx;
                            }
                        }
        ...
        public void AddMissingPrototypesToTerrain(Terrain terrain)
        {
            ...
            foreach (ResourceProtoDetail detail in m_detailPrototypes)
            {
                ...
                foreach (DetailPrototype dp in terrainDetails)
                {
                    ...
                    {
                        if (Gaia.Utils.IsSameTexture(dp.prototypeTexture, detail.m_detailTexture, false))
                        {
                            found = true;
                        }
                        if (Gaia.Utils.IsSameGameObject(dp.prototype, detail.m_detailProtoype, false))
                        {
                            found = true;
                        }
                    }
        ...
        */
        ...
        public GameObject CreateCoverageTextureSpawner(float range)
        {
            ...
            if (terrain != null)
            {
                spawner.m_locationIncrement =
                    Mathf.Clamp(Mathf.Min(terrain.terrainData.size.x, terrain.terrainData.size.z) / (float)terrain.terrainData.alphamapWidth, 0.05f, 1f);
            }
        ...
        public GameObject CreateCoverageDetailSpawner(float range)
        {
            ...
            spawner.m_spawnCollisionLayers = Gaia.TerrainHelper.GetActiveTerrainLayer();
            spawner.m_locationIncrement = 1.2f;
